...
/// Writes only the Dynamic structure/prefix (version + type info) for FLATTENED mode.
///
/// This is called during the prefix phase of serialization.
pub(crate) async fn serialize_dynamic_structure_async<W: ClickHouseWrite>(
    writer: &mut W,
    array: &ArrayRef,
) -> Result<()> {
...
/// Writes only the Dynamic data (indexes + values) for FLATTENED mode.
///
/// This is called during the data phase of serialization.
pub(crate) async fn serialize_dynamic_data_async<W: ClickHouseWrite>(
    writer: &mut W,
    array: &ArrayRef,
...
/// Note: For Nullable(Object), use serialize_dynamic_structure_async and serialize_dynamic_data_async
...
pub(crate) async fn serialize_dynamic_async<W: ClickHouseWrite>(
    ...
    serialize_dynamic_structure_async(writer, array).await?;
    serialize_dynamic_data_async(writer, array, state).await?;
    Ok(())
}

/// Writes only the Dynamic structure/prefix (version + type info) for FLATTENED mode (sync).
pub(crate) fn serialize_dynamic_structure<W: ClickHouseBytesWrite>(
    writer: &mut W,
    array: &ArrayRef,
) -> Result<()> {
    // Determine the ClickHouse type name for non-null values
...
/// Writes only the Dynamic data (indexes + values) for FLATTENED mode (sync).
pub(crate) fn serialize_dynamic_data<W: ClickHouseBytesWrite>(
    writer: &mut W,
    array: &ArrayRef,
    state: &mut SerializerState,
) -> Result<()> {
...
pub(crate) fn serialize_dynamic<W: ClickHouseBytesWrite>(
    ...
    serialize_dynamic_structure(writer, array)?;
    serialize_dynamic_data(writer, array, state)?;
    Ok(())
}
