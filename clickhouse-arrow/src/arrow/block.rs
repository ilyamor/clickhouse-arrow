...
impl ProtocolData<RecordBatch, ArrowDeserializerState> for RecordBatch {
    ...
    async fn write_async<W: ClickHouseWrite>(
        ...
        let batch =
            if options.assemble_json_subcolumns { preprocess_json_subcolumns(self)? } else { self };
        ...
        for (i, field) in schema.fields().iter().enumerate() {
            ...
            if revision >= DBMS_MIN_PROTOCOL_VERSION_WITH_CUSTOM_SERIALIZATION {
                // has_custom_serialization tells ClickHouse to read SerializationInfo
                // (for sparse/nullable encoding), NOT the Object structure version.
                // The Object version is always written as part of the data prefix.
